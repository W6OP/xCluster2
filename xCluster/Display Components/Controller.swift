...
public class  Controller: ObservableObject, TelnetManagerDelegate, WebManagerDelegate {
  ...
  func deleteExistingData(includeSpots: Bool) {
    // TODO: - Probably needs fixing to mark each as deleted
    Task {
      await MainActor.run {
        switch includeSpots {
        case true:
  ...
  func filterOverlays() {
    Task { @MainActor in
        ...
        for spot in unfilteredSpots {
          // TODO: - This needs fixing - need to recreate overlays
          if overlays.first(where: {$0.hashValue == spot.overlayId}) == nil {
            //overlays.append(spot.overlay!)
            var newSpot = spot
            let overlay = newSpot.createOverlay()
            overlays.append(overlay)
          }
        }
        ...
        for spot in filteredSpots {
            overlays.removeAll(where: { $0.hashValue == spot.overlayId })
        }
    }
  }
